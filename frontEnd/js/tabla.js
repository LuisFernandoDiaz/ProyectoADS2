...
let listarHabilidades = async () => {
    ...
    const habilidades = await peticion.json();

    let contenidoTabla = "";

    for (let habilid of habilidades) {
        let contenidoFila = `<tr>
        <td>${habilid.id}</td>
        <td>${habilid.nombre}</td>
        <td>${habilid.lengPro1}</td>
        <td>${habilid.lengPro2}</td>
        <td>${habilid.lengPro3}</td>
        <td>${habilid.baseDato}</td>
        <td>
            <svg  onClick="BorrarFilas(${habilid.id})" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24">
                        <path fill="#ffffff"
                            d="M7 6V3a1 1 0 0 1 1-1h8a1 1 0 0 1 1 1v3h5v2h-2v13a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V8H2V6zm2-2v2h6V4z" />
                    </svg>
                    /
                    <svg  onClick="editarHabilidades(${habilid.id})" xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 1025 1023">
                        <path fill="#ffffff"
...
let BorrarFilas = async (id) => {
    const peticion = await fetch("http://localhost:8080/eliminar/" + id,
        {
            method: "DELETE",
            headers: {
                "Accept": "application/json",
...
let editarHabilidades = async (id) => {

    mostrarFormulario()

    idEditar = id;

    const peticion = await fetch("http://localhost:8080/modificar/" + id,
        {
            method: "PUT",
            headers: {
                "Accept": "application/json",
...
//aplicando
let aplciarActualizacion = async (id) => {
    let campos = {};
    campos.id = id;
